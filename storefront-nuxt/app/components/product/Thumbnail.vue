...
const aspectClass = computed(() => {
  if (props.size === 'square') return 'aspect-[11/14]'
  if (props.size === 'full') return 'aspect-[9/16]'
...
<template>
  ...
  >
    ...
    />
    <div v-else class="w-full h-full absolute inset-0 flex items-center justify-center">
      <UIcon name="i-lucide-image" class="w-8 h-8 text-ui-fg-muted" />
    </div>
  </div>
</template>
